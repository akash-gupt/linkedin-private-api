...
export class SalesNavigatorProfileRequest {
  ...
  getProfile({ profileUrn }: { profileUrn: string }): Promise<unknown> {
  ...
  searchProfiles({
    ...
    if (filters.currentCompanies?.length) {
      const currentCompanies = {
        ...
        values: filters.currentCompanies.map(cc =>
          cc.id ? { id: cc.id, selectionType: 'INCLUDED' } : { text: cc.text, selectionType: 'INCLUDED' },
        ),
      };
    ...
    if (filters.titles?.length) {
      const titles = {
        ...
        values: filters.titles.map(t =>
          t.id ? { id: t.id, text: t.text, selectionType: 'INCLUDED' } : { text: t.text, selectionType: 'INCLUDED' },
        ),
