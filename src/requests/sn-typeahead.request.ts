...
import { RELATIONSHIP_TYPES, SPOTLIGHTS, LANGUAGES } from '../types';
...
export class SalesNavigatorTypeaheadRequest {
  ...
  getLocations({ skip = 0, limit = 25, query }: { skip: number; limit: number; query: string }): Promise<unknown> {
  ...
  getJobTitles({ skip = 0, limit = 25, query }: { skip: number; limit: number; query: string }): Promise<unknown> {
    const url = `${linkedinSalesNavigatorUrl}/salesApiFacetTypeahead`;
  ...
  getCompanies({ skip = 0, limit = 25, query }: { skip: number; limit: number; query: string }): Promise<unknown> {
    const url = `${linkedinSalesNavigatorUrl}/salesApiFacetTypeahead`;
